...
class Edrone():
	...
	def __init__(self):
		...
		self.Kp = [58.98,47.62,55.88]
		self.Ki = [0.00025,0.0035,0.0055]
		self.Kd = [1550,1338.8,795.7]
	...
	#	8. Add error_sum
		...
		print("Drone at  "+self.s+"  Cumulative time: "+str(round(self.duration,2))+"seconds..."+"\r"),
			...
			if(self.cumulative_duration+self.duration >=3):
				if(self.j<6):
					...
					if(self.s == "F6"):
						...
						self.setpoint = self.data[self.s] 
						self.start_time = time.time()
			#elif (self.cumulative_duration+self.duration <=3):
				#print("Drone at  "+self.s+"  Cumulative time: "+str(round(self.duration,2))+"seconds..."+"\r"),
			#print("Drone at  "+self.s+"  Cumulative time: "+str(self.cumulative_duration)+'\r'),			
		elif self.drone_position[0]<=(self.setpoint[0]-0.5) and self.drone_position[0]>=(self.setpoint[0]+0.5) and self.drone_position[1]<=(self.setpoint[1]-0.5) and self.drone_position[1]>=(self.setpoint[1]+0.5) and self.drone_position[2]<=(self.setpoint[2]-0.5) and self.drone_position[2]>=(self.setpoint[2]+0.5):
			self.start_time = time.time()
