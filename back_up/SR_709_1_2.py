...
class Edrone():
	...
	def __init__(self):
		...
		self.Kp = [24.72,18.72,68.1]  #568 #r-3646
		self.Ki = [0.0025,0.002,0]     #13
		self.Kd = [830.5,730.5,897.3]
	...
	#	8. Add error_sum
		...
		if timechange > self.sample_time and self.flag == 1:
			#roll_pid ie y-axis
			...
			self.out_Pitch=self.Kp[1] * self.error[1] +self.Iterm[1] + self.Kd[1] * chg_errorp
			...
			self.out_roll=self.Kp[0] * self.error[0] +self.Iterm[0] + self.Kd[0] * chg_errorR
			...
			self.out_Throttle=self.Kp[2] * self.error[2] +self.Iterm[2] + self.Kd[2] * chg_errorT
		...
		if self.drone_position[0]>=(0.0) and self.drone_position[0]<=(0.4) and self.drone_position[1]>=(-0.2) and self.drone_position[1]<=(0.2) and self.drone_position[2]>=(22.8) and self.drone_position[2]<=(23.2):
			if self.fl[0] == 0:
				print("Reached level 1")
				self.fl[0]=1
			self.setpoint=[3.5, -3.3, 23]

		if self.drone_position[0]>=(3.3) and self.drone_position[0]<=(3.7) and self.drone_position[1]>=(-3.5) and self.drone_position[1]<=(-3.1) and self.drone_position[2]>=(22.9) and self.drone_position[2]<=(23.1):
			if self.fl[1] == 0:
				print("Reached level 2")
				self.fl[1]=1
			self.setpoint=[-4.4, -4.6, 20.7]

		if self.drone_position[0]>=(-4.6) and self.drone_position[0]<=(-4.2) and self.drone_position[1]>=(-4.6) and self.drone_position[1]<=(-4.4) and self.drone_position[2]>=(20.5) and self.drone_position[2]<=(20.9):
			if self.fl[2] == 0:
				print("Reached level 3")
				self.fl[2]=1
			self.setpoint=[-6.0, 6.3, 18.3]

		if self.drone_position[0]>=(-6.2) and self.drone_position[0]<=(-5.8) and self.drone_position[1]>=(6.1) and self.drone_position[1]<=(6.5) and self.drone_position[2]>=(18.1) and self.drone_position[2]<=(18.5):
			if self.fl[3] == 0:
				print("Reached level 4")
				self.fl[3]=1
			self.setpoint=[5.0, 5.3, 16.3]

		if self.drone_position[0]>=(4.8) and self.drone_position[0]<=(5.2) and self.drone_position[1]>=(5.1) and self.drone_position[1]<=(5.5) and self.drone_position[2]>=(16.1) and self.drone_position[2]<=(16.5):
			if self.fl[4] == 0:
